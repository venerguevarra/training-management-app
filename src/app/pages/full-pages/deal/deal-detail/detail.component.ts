...
export class DealDetailComponent {
					...
					this.httpClient.get(`${this.ENDPOINT}/${this.modelId}`).subscribe(
						(data) => {
							...
							if(this.currentModel.accountId != null) {
								this.httpClient.get(`${this.ACCOUNT_ENDPOINT}/${this.currentModel.accountId}`).subscribe(
									(data) => {
							...
							if(this.currentModel.accountId != null) {
								this.httpClient.get(`${this.CONTACT_ENDPOINT}/${this.currentModel.contactId}`).subscribe(
									(data) => {
							...
							if(this.currentModel.courseId != null) {
								this.httpClient.get(`${this.COURSE_ENDPOINT}/${this.currentModel.courseId}`).subscribe(
									(data) => {
							...
							this.currentForm = this.formBuilder.group({
								...
								totalDealAmount: [this.currentModel.totalDealAmount, [Validators.required]],
								courseFee: [this.currentModel.courseFee, [Validators.required]],
								numberOfParticipants: [this.currentModel.numberOfParticipants, [Validators.required]],
	...
	calculateTotalDealAmount() {
		if(this.isControlValueValid(this.f.numberOfParticipants) && this.isControlValueValid(this.f.courseFee)) {
			this.f.totalDealAmount.setValue(parseFloat(this.f.courseFee.value) * parseInt(this.f.numberOfParticipants.value));
		} else {
	...
	update() {
		...
		if(this.f.contactId.value == '' || this.f.contactId.value == 'undefined' || this.f.contactId == null) {
			this.isContactIdInvalid = true;
			hasError = true;
		} else {
			this.isContactIdInvalid = false;
						...
						.subscribe(
							(data) => {
								if(data.status == 201) {
									...
									this.router.navigate(['/app/account', this.parentAccountId], {queryParams: {action: 'view'}});
								}
							},
							(error) => {
								if(error.status === 409) {
